...
const newStation = async (req, res) => {
    ...
    if (!name || !latitude || !longitude || !status) {
        return res.status(400).json({ message: "Debe llenar los campos obligatorios nombre, status, latitud y longitud" })
    }
    ...
    if (station) {
        if (station.name == name) {
            return res.status(400).json({ message: "Ya existe una estación con el mismo nombre" })
        }
        else {
            return res.status(400).json({ message: "Ya existe una estación en las mismas coordenadas" })
        }

    }
    let privateKey;
    await bcrypt.genSalt(saltRounds)
    await bcrypt.hash(name, saltRounds).then(hash =>
        privateKey = hash
    )

    station = await models.Station.create({
        private_key: privateKey,
        name, country, city, latitude, longitude, status
    })
